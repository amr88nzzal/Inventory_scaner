...
export default function Employees() {
  ...
  async function load() {
    try {
      const userList = await api.listUsers().catch(() => []);
      const whList = await api.listWarehouses().catch(() => []);
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || "حدث خطأ أثناء إضافة الموظف. يرجى المحاولة مرة أخرى.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-8">
        ...
        <form onSubmit={handleCreate} className="border border-graphite/15 rounded-sm bg-white p-5 h-fit">
          ...
          {error && (
            <div className="mb-3 text-xs text-warn bg-warn/5 border border-warn/15 rounded-sm p-2 font-medium">
              ⚠️ {error.includes("already taken") ? "اسم المستخدم محجوز مسبقاً" : error}
            </div>
          )}
          <input
