...
export default function DashboardHome() {
  ...
  useEffect(() => {
    (async () => {
      const [items, warehouses, users, pending, taskList] = await Promise.all([
        api.listItems(),
        api.listWarehouses(),
        api.listUsers(),
        api.listPendingItems(),
        api.listTasks(),
      ]);
      setTasks(taskList);
      setStats({
        ...
        tasksNeedingRecount: taskList.filter((t: any) => t.status === "NEEDS_RECOUNT").length,
      });
    })();
  }, []);
